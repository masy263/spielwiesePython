...
def fct_spectrum(sig, fSamp, fftRes=1024):
  ...
  spec  = np.fft.fft(sig, n=fftRes)
  spec  = np.fft.fftshift(10 * np.log10(np.abs(spec)**2))
  t     = np.arange(0, len(spec)) * len(spec) / len(sig) / fSamp
  fAxis = np.fft.fftfreq(t.shape[-1]) * fSamp
